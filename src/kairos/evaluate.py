...
def evaluate(model_path: str = "outputs/adult_model", regression_gate: bool = True):
    ...
    try:
        ...
        print(
            f"❌ Model artifact not found at {model_path}. Run 'python main.py' first."
        )
        sys.exit(1)
    ...
    print("=" * 40)
    print(
        f"Precision (Claim: >96%):      {precision:.4f} {'✅' if precision >= 0.96 else '⚠️'}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-gate", action="store_true", help="Disable regression failure exit codes"
    )
