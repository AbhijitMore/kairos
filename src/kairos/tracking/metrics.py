...
def evaluate_probabilistic_model(y_true: np.ndarray, y_prob: np.ndarray) -> Dict[str, float]:
    """
...
def evaluate_policy(y_true: np.ndarray, probs: np.ndarray, tau_low: float, tau_high: float) -> Dict[str, Any]:
    """
    ...
    if np.sum(covered_mask) > 0:
        accuracy_covered = float(accuracy_score(y_true[covered_mask], preds[covered_mask]))
...
def plot_confusion_matrix(y_true: np.ndarray, y_prob: np.ndarray, tau_low: float, tau_high: float):
    """
