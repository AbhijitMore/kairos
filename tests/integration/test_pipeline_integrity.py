...
def test_pipeline_serialization_reproducibility(trained_engine):
    ...
    np.testing.assert_allclose(original_probs, reloaded_probs, rtol=1e-7, err_msg="Serialized model output deviated from original!")
...
def test_monotonicity_logic(trained_engine):
    ...
    """
    base_instance = {
        'age': 35, 'workclass': 'Private', 'education_num': 8,
    ...
    assert prob_high >= prob_low, f"Monotonicity violation: Education {prob_high} < {prob_low}"
